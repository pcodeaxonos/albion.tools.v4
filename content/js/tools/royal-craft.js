...
function wizardProgressCard(group) {
    ...
    return `
        <article class="royal-wizard-group ${stateClass}">
            <span class="royal-wizard-group-icon ${done ? 'is-complete' : active ? 'is-active' : ''}" aria-hidden="true">${done ? '<img src="content/icons/royal-wizard/check.svg" alt="">' : `<svg viewBox="0 0 36 36" focusable="false"><circle class="royal-wizard-group-track" cx="18" cy="18" r="15"/>${active ? `<circle class="royal-wizard-group-fill" cx="18" cy="18" r="15" pathLength="100" stroke-dasharray="${(group.complete.length / group.total) * 100} 100" transform="rotate(-90 18 18)"/>` : ''}</svg>`}</span>
            <div>
...
function renderWizardHeader() {
    return `<header class="royal-wizard-header">
        <img class="royal-wizard-crown" src="content/icons/royal-wizard/crown.svg" alt="">
        <div><h2>${ROYAL_WIZARD_TITLE}</h2><p>${ROYAL_WIZARD_DESCRIPTION}</p></div>
        <button type="button" class="royal-wizard-help" data-wizard-help><img src="content/icons/royal-wizard/info.svg" alt="">Nasıl çalışır?</button>
    </header>`;
}
    ...
    return renderWizardShell({
        ...
        body: `
            ...
            <aside class="royal-wizard-aside">
                <section><h3><img src="content/icons/royal-wizard/info.svg" alt="">Hazırlık kuralı</h3><p>Royal item fiyatları hesaplanmadan önce gerekli tüm temel fiyat grupları (Sigil, Rune/Soul/Relic ve malzemeler) güncellenir. Bu veriler, hesaplamaların doğru ve kararlı olması için kullanılır.</p></section>
                <section><h3><img src="content/icons/royal-wizard/crown.svg" alt="">Geçiş kuralı</h3><p>Tüm gerekli fiyat grupları hazır olduğunda sihirbaz otomatik olarak bir sonraki adıma geçer. Herhangi bir işlem yapmana gerek yoktur.</p></section>
            </aside>
        ...
        footer: `<span><img src="content/icons/royal-wizard/info.svg" alt="">${isReady ? 'Tamamlandığında otomatik ilerler' : `${total - complete} fiyat hâlâ eksik.`}</span>
...
function renderRoyalWizardStepTwo(run) {
    ...
    return renderWizardShell({
        ...
        footer: `<button type="button" class="btn btn-outline-secondary royal-wizard-back" data-wizard-back><img src="content/icons/royal-wizard/back.svg" alt="">Geri</button>
